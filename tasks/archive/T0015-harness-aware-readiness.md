---
id: T0015
title: Harness-aware lane readiness markers
status: done
accepted: 2026-06-13
depends_on: []
...
# T0015 — Harness-aware lane readiness markers
...
Add working_marker and idle_marker registry fields (extended-regex; empty-safe). loop-lane-status.sh PREFERS the harness's declared markers when @loop_lane_harness is set, falling back to today's heuristics otherwise. Populate markers for the known harnesses (claude 'accept edits on|bypass permissions on'; codex 'esc to interrupt' + the model/cwd footer; pi braille; shell themed prompts). FROZEN: the single-word status output (working|awaiting-approval|idle|errored|unknown) is unchanged, and EVERY existing special case (codex spinner-above-composer, themed shell prompts, Pi footer, bare prompt) must still classify identically. Add fixture/tmux-pane tests proving no regression.

## Context you need
Files: lib/harness-registry.sh (working_marker / idle_marker fields); loop-lane-status.sh; tests.
...
tests pass UNCHANGED. Commit the batch per the commit policy (cite T0015). Do NOT
